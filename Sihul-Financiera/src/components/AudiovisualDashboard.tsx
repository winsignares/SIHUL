...
import EstadoRecursos from './admin/EstadoRecursos';
import OcupacionSemanal from './admin/OcupacionSemanal';
import PlazasDisponibles from './admin/PlazasDisponibles';
...
type MenuOption = 'home' | 'prestamos' | 'recursos' | 'plazas' | 'ocupacion' | 'notificaciones' | 'ajustes';
...
export default function AudiovisualDashboard({ userName, onLogout }: AudiovisualDashboardProps) {
  ...
  const menuSections = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'recursos', icon: Boxes, label: 'Estado de Recursos', action: 'recursos' }
      ]
    },
    {
      id: 'visualizacion',
      label: 'VISUALIZACIÓN',
      items: [
        { id: 'ocupacion', icon: BarChart3, label: 'Visualización de Ocupación', action: 'ocupacion' },
        { id: 'plazas', icon: MapPin, label: 'Plazas Disponibles', action: 'plazas' }
      ]
    }
  ];
  ...
  const renderContent = () => {
    switch (activeMenu) {
      case 'home':
        return <AudiovisualHome onNavigate={setActiveMenu} />;
      case 'prestamos':
