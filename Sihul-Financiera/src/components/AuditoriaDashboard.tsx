...
import { LayoutDashboard, LogOut, Search, Settings, Menu, ChevronDown, ChevronRight, ChevronLeft, Shield, FileSearch, Clock } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { Input } from './ui/input';
import { Avatar, AvatarFallback } from './ui/avatar';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from './ui/tooltip';
import AuditoriaHome from './auditoria/AuditoriaHome';
import ControlPrevio from './auditoria/ControlPrevio';
import MisPendientes from './auditoria/MisPendientes';
...
type MenuOption = 'home' | 'control' | 'pendientes';

export default function AuditoriaDashboard({ userName, onLogout }: AuditoriaDashboardProps) {
  ...
  const menuSections = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'control', icon: FileSearch, label: 'Control Previo', action: 'control' }
      ]
    }
  ];
  ...
  const renderContent = () => {
    switch (activeMenu) {
      case 'home':
        return <AuditoriaHome onNavigate={setActiveMenu} />;
      case 'control':
          ...
          <div className="p-6 border-b border-red-700/50 flex items-center justify-center">
            <motion.div className="flex items-center gap-3" animate={{ justifyContent: isSidebarCollapsed ? 'center' : 'flex-start' }} whileHover={{ scale: 1.02 }}>
              <div className="w-12 h-12 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-xl flex items-center justify-center shadow-lg flex-shrink-0">
                <Shield className="w-7 h-7 text-red-900" />
              </div>
              <AnimatePresence>
                {!isSidebarCollapsed && (
                  <motion.div initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} transition={{ duration: 0.3 }}>
                    ...
                    <p className="text-red-300 whitespace-nowrap">Auditoría</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.div>
                          ...
                          {section.items.map((item) => {
                            ...
                            const buttonContent = (
                              <motion.button key={item.id} onClick={() => setActiveMenu(item.action as MenuOption)} className={`w-full flex items-center gap-3 rounded-xl transition-all duration-200 group ${isSidebarCollapsed ? 'justify-center px-3 py-3' : 'px-4 py-3'} ${isActive ? 'bg-gradient-to-r from-yellow-400 to-yellow-500 text-red-900 shadow-lg' : 'text-red-100 hover:bg-red-700/50'}`} whileHover={{ x: isActive ? 0 : (isSidebarCollapsed ? 0 : 4), scale: isSidebarCollapsed ? 1.05 : 1 }} whileTap={{ scale: 0.95 }}>
                                ...
                                <AnimatePresence>
                                  {!isSidebarCollapsed && (
                                    <motion.span initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} transition={{ duration: 0.2 }} className="flex-1 text-left whitespace-nowrap overflow-hidden">
                                      {item.label}
                                    </motion.span>
                                  )}
                                </AnimatePresence>
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400">Sistema de Auditoría</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <motion.div whileHover={{ scale: 1.02 }} className="flex items-center gap-3 px-3 py-2 rounded-xl bg-gradient-to-r from-red-50 to-slate-50 border border-red-100">
                  ...
                  <div className="hidden lg:block">
                    ...
                    <p className="text-slate-500">Auditoría</p>
                  </div>
                </motion.div>
              </div>
            </div>
