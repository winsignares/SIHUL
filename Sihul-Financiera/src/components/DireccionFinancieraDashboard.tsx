...
import { LayoutDashboard, LogOut, Search, Settings, Menu, ChevronDown, ChevronRight, ChevronLeft, Briefcase, FileCheck, Send, CheckCircle, BarChart3, Clock } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { Input } from './ui/input';
import { Avatar, AvatarFallback } from './ui/avatar';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from './ui/tooltip';
import DireccionFinancieraHome from './direccion-financiera/DireccionFinancieraHome';
import RevisarPagos from './direccion-financiera/RevisarPagos';
import EnviarRectoria from './direccion-financiera/EnviarRectoria';
import ConfirmacionPagos from './direccion-financiera/ConfirmacionPagos';
import MisPendientes from './direccion-financiera/MisPendientes';

interface DireccionFinancieraDashboardProps {
  userName: string;
  onLogout: () => void;
}

type MenuOption = 'home' | 'revisar' | 'enviar' | 'confirmar' | 'pendientes';

export default function DireccionFinancieraDashboard({ userName, onLogout }: DireccionFinancieraDashboardProps) {
  ...
  const menuSections = [
    ...
    { id: 'gestion', label: 'Gestión de Pagos', items: [
      { id: 'pendientes', icon: Clock, label: 'Mis Pendientes', action: 'pendientes' },
      { id: 'revisar', icon: FileCheck, label: 'Revisar Pagos', action: 'revisar' },
      { id: 'enviar', icon: Send, label: 'Enviar a Rectoría', action: 'enviar' },
      { id: 'confirmar', icon: CheckCircle, label: 'Confirmación de Pagos', action: 'confirmar' }
    ]}
  ];
  ...
  const renderContent = () => {
    switch (activeMenu) {
      case 'home': return <DireccionFinancieraHome onNavigate={setActiveMenu} />;
      case 'revisar': return <RevisarPagos />;
      case 'enviar': return <EnviarRectoria />;
      case 'confirmar': return <ConfirmacionPagos />;
      case 'pendientes': return <MisPendientes />;
      default: return <DireccionFinancieraHome onNavigate={setActiveMenu} />;
    }
  };
          ...
          <div className="p-6 border-b border-red-700/50 flex items-center justify-center">
            <motion.div className="flex items-center gap-3" animate={{ justifyContent: isSidebarCollapsed ? 'center' : 'flex-start' }} whileHover={{ scale: 1.02 }}>
              <div className="w-12 h-12 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-xl flex items-center justify-center shadow-lg flex-shrink-0">
                <Briefcase className="w-7 h-7 text-red-900" />
              </div>
              <AnimatePresence>
                {!isSidebarCollapsed && (
                  <motion.div initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} transition={{ duration: 0.3 }}>
                    ...
                    <p className="text-red-300 whitespace-nowrap text-xs">Dir. Financiera</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.div>
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400 text-sm">Dirección Financiera / Sindicatura</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <motion.div whileHover={{ scale: 1.02 }} className="flex items-center gap-3 px-3 py-2 rounded-xl bg-gradient-to-r from-red-50 to-slate-50 border border-red-100">
                  ...
                  <div className="hidden lg:block">
                    ...
                    <p className="text-slate-500 text-xs">Dir. Financiera</p>
                  </div>
                </motion.div>
              </div>
            </div>
