...
import TesoreriaHome from './tesoreria/TesoreriaHome';
import AlistarPagos from './tesoreria/AlistarPagos';
import RegistrarPagoAplicado from './tesoreria/RegistrarPagoAplicado';
import MisPendientes from './tesoreria/MisPendientes';
import EnviarDireccionFinanciera from './tesoreria/EnviarDireccionFinanciera';
import GenerarComprobanteEgreso from './tesoreria/GenerarComprobanteEgreso';

interface TesoreriaDashboardProps {
  userName: string;
  onLogout: () => void;
}

type MenuOption = 'home' | 'alistar' | 'enviar' | 'comprobante' | 'registrar' | 'pendientes';

export default function TesoreriaDashboard({ userName, onLogout }: TesoreriaDashboardProps) {
  ...
  const menuSections = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'alistar', icon: FileCheck, label: 'Alistar Pagos', action: 'alistar' },
        { id: 'enviar', icon: Send, label: 'Enviar a Dirección Financiera', action: 'enviar' },
        { id: 'registrar', icon: CheckCircle, label: 'Registrar Pago Aplicado', action: 'registrar' },
        { id: 'comprobante', icon: FileOutput, label: 'Generar Comprobante de Egreso', action: 'comprobante' }
      ]
    }
  ];
  ...
  const renderContent = () => {
    switch (activeMenu) {
      case 'home':
        return <TesoreriaHome onNavigate={setActiveMenu} />;
      case 'alistar':
        return <AlistarPagos />;
      case 'registrar':
          ...
          <div className="p-6 border-b border-red-700/50 flex items-center justify-center">
            ...
            >
              <div className="w-12 h-12 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-xl flex items-center justify-center shadow-lg flex-shrink-0">
                <Wallet className="w-7 h-7 text-red-900" />
              </div>
              <AnimatePresence>
                {!isSidebarCollapsed && (
                  <motion.div
                  ...
                  >
                    ...
                    <p className="text-red-300 whitespace-nowrap">Tesorería</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.div>
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-500 dark:text-slate-400">Sistema de Tesorería</p>
                </div>
              </div>
              ...
              <div className="hidden md:flex flex-1 max-w-xl">
                <div className="relative w-full">
                  ...
                  <Input
                    placeholder="Buscar facturas, comprobantes..."
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  <div className="hidden lg:block">
                    ...
                    <p className="text-slate-500">Tesorería</p>
                  </div>
                </motion.div>
              </div>
            </div>
