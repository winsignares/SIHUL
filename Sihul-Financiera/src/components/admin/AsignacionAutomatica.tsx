...
export default function AsignacionAutomatica() {
  ...
  const procesarArchivo = async (file: File) => {
    try {
      ...
      toast.success(`✅ ${horariosParseados.length} horarios cargados\n📚 ${programas.length} programa(s)\n👥 ${grupos.length} grupo(s)`);
    } catch (error) {
  ...
  const iniciarAsignacionAutomatica = async () => {
    ...
    if (fallidos === 0) {
      toast.success(`🎉 ¡Asignación completada! ${exitosos} horarios asignados correctamente`);
    } else {
      toast.warning(`⚠️ Asignación completada con advertencias: ${exitosos} exitosos, ${fallidos} fallidos`);
    }
  };
    ...
    try {
      ...
      if (!programa) {
        ...
        programa = db.createPrograma({
          ...
          email: 'programa@unilibre.edu.co',
          telefono: '000-0000',
  ...
  const descargarPlantilla = () => {
    ...
    toast.success('✅ Plantilla descargada - Horario completo por grupo');
  };
  ...
  const confirmarAsignacion = (index: number) => {
    ...
    toast.success('✅ Asignación confirmada correctamente');
  };
  ...
  const confirmarGrupoCompleto = () => {
    ...
    toast.success(`✅ Horario completo del grupo ${grupoSeleccionado} confirmado`);
  };
  ...
  const rechazarGrupoCompleto = () => {
    ...
    setAsignacionesRechazadas(nuevasRechazadas);
    setMostrarHorarioCompleto(false);
    toast.success(`❌ Horario completo del grupo ${grupoSeleccionado} rechazado`);
  };
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    toast.success('Horario visualizado correctamente');
                  }}
                >
                  <CheckCircle2 className="w-4 h-4 mr-2" />
                  Entendido
