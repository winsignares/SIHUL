...
import type { HorarioAcademico, Facultad, Programa, EspacioFisico, Grupo } from '../../lib/models';
...
export default function CentroHorarios() {
  const [activeTab, setActiveTab] = useState<'consulta' | 'modificacion'>('consulta');
  ...
  const eliminarSeleccionados = () => {
    if (horariosSeleccionados.size === 0) {
      toast.error('No hay horarios seleccionados');
      return;
    }
    ...
    if (confirm(`¿Está seguro de eliminar ${horariosSeleccionados.size} horario(s)?`)) {
      ...
      if (eliminados > 0) {
        toast.success(`${eliminados} horario(s) eliminado(s) correctamente`);
        loadData();
        setHorariosSeleccionados(new Set());
        setSeleccionarTodos(false);
      } else {
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      if (confirm(`¿Está seguro de eliminar todo el horario del grupo ${grupo.grupo}? Se eliminarán ${grupo.horarios.length} clases.`)) {
                                        ...
                                        if (eliminados > 0) {
                                          toast.success(`Grupo ${grupo.grupo} eliminado correctamente (${eliminados} clases)`);
                                          loadData();
                                        } else {
