...
export default function SupervisorSalonHome() {
  ...
  const buscarSalones = () => {
    if (!sedeSeleccionada || !pisoSeleccionado) {
      toast.error('Por favor selecciona Sede y Piso', {
        description: 'Debes completar los filtros antes de buscar'
      });
      return;
    }
    setBusquedaActiva(true);
    toast.success('Búsqueda realizada', {
      description: `${sedeSeleccionada} - Piso ${pisoSeleccionado} - ${horaSeleccionada}`,
  ...
  const abrirSalon = (espacioId: number, nombreSalon: string) => {
    ...
    toast.success(`✅ Salón ${nombreSalon} abierto`, {
      description: `Hora de apertura: ${obtenerHoraActual()}`,
  ...
  const cerrarSalon = () => {
    ...
    if (!todoCompleto) {
      toast.error('Checklist incompleto', {
        description: 'Debes verificar todos los ítems antes de cerrar el salón'
      });
      return;
    }
    ...
    toast.success(`🔒 Salón ${salonParaCerrar.nombre} cerrado correctamente`, {
      description: `Hora de cierre: ${obtenerHoraActual()}${checklist.observaciones ? ' - Con observaciones' : ''}`,
