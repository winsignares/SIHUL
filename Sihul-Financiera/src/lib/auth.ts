...
export class AuthService {
  ...
  static login(email: string, password: string): LoginResult {
    // Validar formato de email - Aceptar @unilibre.edu.co Y @financiera.edu.co
    const isUnilibreEmail = email.endsWith('@unilibre.edu.co');
    const isFinancieraEmail = email.endsWith('@financiera.edu.co');
    
    if (!isUnilibreEmail && !isFinancieraEmail) {
      return {
        success: false,
        error: 'El correo debe ser institucional (@unilibre.edu.co o @financiera.edu.co)'
      };
    }
