...
export default function GruposFusion() {
  ...
  const handleCrearFusion = () => {
    ...
    if (selectedGrupos.length < 2) {
      showNotification({ message: 'Debe seleccionar al menos 2 grupos para fusionar', type: 'error' });
      return;
    }

    if (!asignaturaComun) {
      showNotification({ message: 'Debe seleccionar una asignatura común', type: 'error' });
      return;
    }

    if (!espacioAsignado) {
      showNotification({ message: 'Debe asignar un espacio físico', type: 'error' });
      return;
    }

    if (capacidadValidacion && !capacidadValidacion.suficiente) {
      showNotification({ message: 'El espacio seleccionado no tiene capacidad suficiente', type: 'error' });
      return;
    }

    if (!estadoValidacion.todosActivos) {
      showNotification({ message: 'No se pueden fusionar grupos inactivos', type: 'error' });
      return;
    }
    ...
    showNotification({ message: 'Fusión de grupos creada exitosamente', type: 'success' });
  ...
  const handleEliminarFusion = (fusionId: string) => {
    ...
    showNotification({ message: 'Fusión eliminada exitosamente', type: 'success' });
  };
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={isFusionDialogOpen} onOpenChange={setIsFusionDialogOpen}>
          <DialogTrigger asChild>
            <Button className="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white ml-4">
