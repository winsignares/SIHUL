"""
Script to reorganize horarios by physical space
"""
...
SEDE_PRINCIPAL_SPACES = [
    ...
    'SALON 203A', 'SALON 204A', 'SALON 205B', 'SALON 206B', 'SALON 301B',
...
def extract_schedules_from_text(text, start_marker, end_marker):
    """Extract schedule tuples from text between markers"""
    ...
    # Extract tuples using regex - handles empty strings and quotes with apostrophes
    # More flexible pattern that allows empty strings
    pattern = r"\((['\"])([^'\"]*)\1\s*,\s*(['\"])([^'\"]*)\3\s*,\s*(['\"])([^'\"]*)\5\s*,\s*(['\"])([^'\"]*)\7\s*,\s*(['\"])([^'\"]*)\9\s*,\s*(['\"])([^'\"]*)\11\s*,\s*(['\"])([^'\"]*)\13\s*\)"
    
    # Simpler approach: split by lines and parse each tuple manually
    schedules = []
    lines = array_text.split('\n')
    
    for line in lines:
        line = line.strip()
        # Skip comments and non-tuple lines
        if not line.startswith('(') or line.startswith('#'):
            continue
        
        # Try to extract the 7 tuple elements
        try:
            # Remove the outer parentheses and trailing comma
            line = line.strip('(),')
            ...
            if len(parts) == 7:
                grupo, materia, profesor, dia, hora_inicio, hora_fin, espacio = parts
...
# Extract schedules from both arrays
print("Extracting schedules from Sede Centro...")
centro_schedules = extract_schedules_from_text(content, "def create_horarios_sede_centro", "def create_horarios_sede_principal")

print("Extracting schedules from Sede Principal...")
principal_schedules = extract_schedules_from_text(content, "def create_horarios_sede_principal", "# Reconectar la validación")
...
def format_schedules_for_sede(grouped_schedules, space_list, sede_name):
    ...
    unknown_spaces = sorted([s for s in grouped_schedules.keys() if s not in space_list])
